%ZHIRAN: MOST UP TO DATE SCANNING CODE JAN 14 2020 ( "Spiral" curve, scanning)
...
Force_set_pnt = 19.85e-3; %chnage this each time. used only during first FAM scan, %ZHIRAN,MAKE SURE SET POINT IS RIGHT
...
xy_points(1,1)=0;xy_points(1,2)=0;    
i=1;
dlta=abs(x_points(1)-x_points(2));
for iround=1:(nx-1)/2
    i=i+1;
    xy_points(i,1)=xy_points(i-1,1)+dlta;xy_points(i,2)=xy_points(i-1,2);
    for istep=1:dlta*(2*iround-1)
        i=i+1;
        xy_points(i,1)=xy_points(i-1,1);xy_points(i,2)=xy_points(i-1,2)+dlta;
    end
    for istep=1:dlta*(2*iround)
        i=i+1;
        xy_points(i,1)=xy_points(i-1,1)-dlta;xy_points(i,2)=xy_points(i-1,2);
    end
    for istep=1:dlta*(2*iround)
