function scan_xy_T1_0t0_0tP_nPoint_adaptiveTau_SCC_ConfocalReg( )
...
global driftingdata %create this variable manually in matlab before starting this program.!!!!!!!!!!!!!!!!!!!!
...
pixels_y = 80;
periodregi=10;% set how frequetly for image registration, in seconds
iffirstrun=1;
cyclenum=1;
...
x_points = linspace(-10,10,5);
y_points = linspace(-10,10,5); %ZHIRAN, sets scan window, in nanometers
...
numb_repeats = 3; %ZHIRAN, must be same as in ESR GUI
...
for scan_repeat_numb = 1:scan_repeats
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if etime(nowclock,previousclock)> periodregi
            ...
            driftingdata(cyclenum,2)=drift_nm_x; %record the drifting x
            driftingdata(cyclenum,3)=drift_nm_y; %record the drifting y
            driftingdata(cyclenum,4)=outTotal*1.25; %record the drifting z
            cyclenum=cyclenum+1;
            
            % now incremnt x , y to new drifted points
            %if abs(drift_nm_x)<1000 && abs(drift_nm_y)<1000
            %    'Move the tip.'
            ...
            %    mDAC('set_scan_center_y',initial_pos(2));
            %else
            %    'The drift seems to be bigger than 1um in X/Y, so not moving tip.'
            %end
        end
