function scan_ps_pillar_x_range_T_ptp_0t0_nPoint_AFMreg( )
...
Force_set_point = 28.1e-3 ; %force set point for the initial AFM scan
AFM_track_number = 62; % 
...
scan_size_x = 0.025;
scan_size_y = 0.025;
%set scan size to 250nm in x
mDAC('set_scan_size_x', scan_size_x);

%set scan size to 250nm in y
...
afm_scan_x_points_numb = 100;
afm_scan_y_points_numb = 100;
...
yrange_ns_omega = ones(1,41)*(3000e3+1000);
...
forceSetpt = 28.1e-3 ; %27.8mV set point
...
numTauPoints = 5; % same as in ESR GUI
numc=2;
...
for ka=1:(nx_all*numSequences)
 ...
 % ************** code to re take AFM image 
   ...
   % and engage
   mDAC('move_tip_laser', -xpts_nm(k) -scan_size_x*10000/2,-scan_size_y*10000/2,0,0);
   pause(10);
