...
public:
	...
	void define_tasks();//Define the channels to be read (isn't used by magnetometer currently)
	...
	float phase_check(); // read the phase from lockin as voltage
	
	void define_phase_task(); // NI DAQ task for phase analog input
...
//	double freq_current; //Current voltage output for PLL frequency set point
	...
	TaskHandle approach_handle,sweep_handle,sweep_count, phase_handle; 
