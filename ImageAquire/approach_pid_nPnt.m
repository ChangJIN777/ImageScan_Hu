...
currentDefaultOut = 10.000; % V, NOTE THAT here 10 V is 12.5 micron in nPoint.
outCenter = 0.0; %V, 
outRange = 10; % 
outMaxApproach = -10.0; % something close to but less than outRange
...
DACzMax = 10.0; % voltas -10V to +10 is 25 µm on nPoint
DACzStep = 2.1; % 2.10V = 21.0 µm step size, smaller than  outMaxApproach
...
while(~isApproached)
    ...
    outTotal = mean(outSamples);% so outTotal is now the voltage value (where 10V=12.5 micron) of the
    % PID output going to nPoint. 
    % so to back off some 1250 nm requires default out to be increased 1V
