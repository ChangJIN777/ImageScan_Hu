function varargout = ESRControlBackup(varargin)
% ESRCONTROLBACKUP MATLAB code for ESRControlBackup.fig
%      ESRCONTROLBACKUP, by itself, creates a new ESRCONTROLBACKUP or raises the existing
%      singleton*.
%
%      H = ESRCONTROLBACKUP returns the handle to a new ESRCONTROLBACKUP or the handle to
%      the existing singleton*.
%
%      ESRCONTROLBACKUP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ESRCONTROLBACKUP.M with the given input arguments.
%
%      ESRCONTROLBACKUP('Property','Value',...) creates a new ESRCONTROLBACKUP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ESRControlBackup_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ESRControlBackup_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ESRControlBackup

% Last Modified by GUIDE v2.5 11-Jan-2018 20:03:44

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ESRControlBackup_OpeningFcn, ...
                   'gui_OutputFcn',  @ESRControlBackup_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ESRControlBackup is made visible.
function ESRControlBackup_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ESRControlBackup (see VARARGIN)



% Choose default command line output for ESRControlBackup
...
global esr_handles;
esr_handles = handles;
% UIWAIT makes ESRControlBackup wait for user response (see UIRESUME)
% uiwait(handles.figure1);
end

% --- Outputs from this function are returned to the command line.
function varargout = ESRControlBackup_OutputFcn(hObject, eventdata, handles) 
...
while (kC <= numCommands) && (handles.stopAutomation==0)
    ...
    switch(baseCommand)
        ...
        case 'CWESR_measurement' 
                ...
                if runMode==1
                    % first initialize settings on the esrcontrolbackup GUI
                    ...
                    % argument is 'esrGUI' which is esrcontrolbackup handles
                    handles.EsrCWSweep.PerformCWESR(handles);
        ...
        case 'Pulsed_measurement'
            logid = fopen(logfile,'a');
                ...
                if runMode==1
                    ...
                    % one separate function for all of esrcontrolbackup that has
...
% pass to the Auto Editor Gui the ESRControlBackup Handles 
