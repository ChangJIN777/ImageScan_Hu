function scan_xy_T1_0t0_0tP_nPoint_AFMreg_adaptiveTau( )
...
Force_set_pnt = 45.3e-3; %chnage this each time. used only during first AFM scan
...
x_points = linspace(-50,50,51);
%x_points = linspace(-400,400,81);
y_points = linspace(-50,50,51);
    ...
    if (bTipTrackXY)
     ...
     if (mod(AFM_galvo_track,galvoTrackPeriod)==0)
       ...
       ['pulse sequence ', num2str(AFM_galvo_track), ' started']
        ...
        C = mean(scanTauPLTriplets_all(12:12:numb_repeats*12)) - mean(scanTauPLTriplets_all(2:12:12*numb_repeats));
        Sig = mean (scanTauPLTriplets_all(11:12:12*numb_repeats)) - mean (scanTauPLTriplets_all(8:12:12*numb_repeats));
        
        decay_frac = Sig/C;
        
        if (decay_frac > 0.7)
            current_point = current_point-1;          
            new_tau_value = new_tau_value/(1.3*1000);
        ...
        elseif (decay_frac <0.2)
            ...
            %do adaptive tau based on current pixel T1
                ...
                C2 = mean (scanTauPLTriplets_all(11:12:12*numb_repeats )) - mean (scanTauPLTriplets_all(2:12:12*numb_repeats));
                Sig2 = mean (scanTauPLTriplets_all(8:12:12*numb_repeats)) - mean(scanTauPLTriplets_all(5:12:12*numb_repeats));
                
                T1 = tau_value/ (log(C2) - log (Sig2)); %in microseconds
