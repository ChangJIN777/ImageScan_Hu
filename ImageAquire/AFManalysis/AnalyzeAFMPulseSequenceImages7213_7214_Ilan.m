...
doLimNormTo_01=1; % whether to limit colorscale on iChan=1 (usually norm) to [0,1] interval

%-------------- inputs---------
scanNum = 7179; % scan number in scans directory
npc  = 1;%5 % number of pulse channels, e.g. = 5 for norm,sig1,ref1,sig2,ref2
...
if (ncc+npc >0)
figure('Position',[400,700,200*(npc+ncc),400]);
...
for iChan=1:npc
    ...
    subplot(3,npc+ncc,iChan+npc+ncc);
    if iChan==1 && doLimNormTo_01
        clims = [0 1];
        imagesc([xmin_um,xmax_um],...
            [ymin_um,ymax_um],...
    ...
    chanImages{3,iChan}=chanImages{1,iChan}+chanImages{2,iChan};
    plottedImage = imresize(chanImages{3,iChan},fac);
    subplot(3,npc+ncc,iChan+npc+ncc+1);
    if iChan==1 && doLimNormTo_01
        clims = [0 2];
        imagesc([xmin_um,xmax_um],...
            [ymin_um,ymax_um],...
