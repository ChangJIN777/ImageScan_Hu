...
%-------------- inputs---------
scanNum = 7277; % scan number in scans directory
...
if doHeightSurfAndContours
    ...
    surf(xpixels,ypixels,-zImages{2,2}*10,'EdgeColor','None');
    colormap('pink')
    colorbar
    ...
    % scans
    plImages = cell(2,1);
    plImages{1,1} = reshape(scanDat(:,end-1),numPointsY,numPointsX); %forward
    plImages{2,1} = reshape(scanDat(:,end),numPointsY,numPointsX); %reverse
    h4 = figure;
    %subplot(1,2,1)
    surf(xpixels,ypixels,plImages{1,1},'EdgeColor','None');
