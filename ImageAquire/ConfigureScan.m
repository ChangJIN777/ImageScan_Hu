...
% Zhiran,7/23/2019. This is added for change the parameters for image
% registration.
  global ConfcScanParm_handles;
  ConfcScanParm_handles = handles;
...
function [varargout] = ConfigureScanFunctions(varargin)
    ...
    switch task,
        ...
        case 'Save'
            ...
            %DAQMaganaer is a class defined in DAQManager.m, its X,Y,Z are just
            %constant 1,2,3. So this is just defining MinVal, etc. in a
            %usual way. Zhiran 7/24/19
...
%             if bEnable(DAQManager.X)
%                 NPoints(DAQManager.X) = str2num(get(handles.pointsX,'String'));
%             else
%                 NPoints(DAQManager.X) = 1;
%             end
%             if bEnable(DAQManager.Y)
%                 NPoints(DAQManager.Y) = str2num(get(handles.pointsY,'String'));
%             else
%                 NPoints(DAQManager.Y) = 1;
%             end
%             if bEnable(DAQManager.Z)
%                 NPoints(DAQManager.Z) = str2num(get(handles.pointsZ,'String'));
%             else
%                 NPoints(DAQManager.Z) = 1;
%             end

%Save the parameters for confocal scan in the handles.ConfocalScan for
%later use (when you start confocal scanning) Zhiran 7/24/19
...
function bool = validate_offset(dim,handles)
    ...
    if enable == true
        ...
        mpv(1) = handles.imageScanHandles.configS.xScanMicronsPerVolt;
        mpv(2) = handles.imageScanHandles.configS.yScanMicronsPerVolt;
        mpv(3) = handles.imageScanHandles.configS.zScanMicronsPerVolt;
        if (input+maxVal) > handles.imageScanHandles.DAQManager.AnalogOutMaxVoltages(dim)*mpv(dim)
            errordlg(['Offset ' dimStr(dim) '+ Max ' dimStr(dim) ' cannot be greater than ' ...
                sprintf('%.2f',handles.imageScanHandles.DAQManager.AnalogOutMaxVoltages(dim)*mpv(dim))],'Input Error');
            bool = false;
            %set(hMax(dim), 'String', num2str(handles.imageScanHandles.DAQManager.AnalogOutMaxVoltages(dim) - maxVal));
        elseif (input+minVal) < handles.imageScanHandles.DAQManager.AnalogOutMinVoltages(dim)*mpv(dim)
            errordlg(['Offset ' dimStr(dim) '+ Min ' dimStr(dim) ' cannot be less than ' ...
                sprintf('%.2f',handles.imageScanHandles.DAQManager.AnalogOutMinVoltages(dim)*mpv(dim))],'Input Error');
            bool = false;
            %set(hMax(dim), 'String', num2str(handles.imageScanHandles.DAQManager.AnalogOutMinVoltages(dim) - minVal));
        end
...
function bool = validate_min(dim,handles)
    ...
    if enable == true
        ...
        if isempty(maxVal) == false && input >= maxVal;
            ...
            bool = false;
            %set(hMin(dim), 'String', get(hMax(dim), 'String'));
        elseif input < handles.imageScanHandles.DAQManager.AnalogOutMinVoltages(dim)*mpv(dim)
            errordlg(['Min ' dimStr(dim) ' cannot be less than ' ...
                sprintf('%.2f',handles.imageScanHandles.DAQManager.AnalogOutMinVoltages(dim)*mpv(dim))],'Input Error');
            bool = false;
            %set(hMin(dim), 'String', num2str(handles.imageScanHandles.DAQManager.AnalogOutMinVoltages(dim)));
        end
    elseif isempty(input)
        set(hMin(dim), 'String', '0');
    end
...
function bool = validate_max(dim,handles)
    ...
    if enable == true
        ...
        mpv(2) = handles.imageScanHandles.configS.yScanMicronsPerVolt;
        mpv(3) = handles.imageScanHandles.configS.zScanMicronsPerVolt;
        if input > handles.imageScanHandles.DAQManager.AnalogOutMaxVoltages(dim)*mpv(dim)
            errordlg(['Max ' dimStr(dim) ' cannot be more than ' ...
                sprintf('%.2f',handles.imageScanHandles.DAQManager.AnalogOutMaxVoltages(dim)*mpv(dim))],'Input Error');
            bool = false;
            %set(hMax(dim), 'String', num2str(DAQManager.AnalogOutMaxVoltages(dim)));
        end
    elseif isempty(input)
        set(hMax(dim), 'String', '0');
    end
...
function bool = validate_points(dim,handles)
    ...
    %set(hPoints(dim), 'String', num2str(abs(round(input))));
end
...
function bool = validate_dwell(handles)
    ...
    if isempty(input)
        ...
        bool = false;
        %set(handles.dwell, 'String', '0');
    end
    if input <= 0
        errordlg('Dwell time must be positive','Input Error');
        bool = false;

        %set(handles.dwell, 'String', num2str(abs(input)));
    end
