...
%%
igorDateFull = 'C:\Users\lab\Documents\Data\2016_Mar_04\';%get(handles.esrSavePath,'String');
igorDateTrunc = igorDateFull((end-6):(end-1));
latestIndex = num2str(7);%num2str(str2num(get(handles.numCompleted,'String'))-2);
...
for i=1:numgraphs
    if i > 9
        igorFolderName = [prefix num2str(i+offset) suffix '000' num2str(i+offset)];
    else
        igorFolderName = [prefix num2str(i+offset) suffix '0000' num2str(i+offset)];      
    end
    x1= num2str(xmat(i));
