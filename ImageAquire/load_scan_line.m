...
for i=start:fin
    ...
    val_2t = (d(5)-d(6)-val_zero)/(d(6));
     ...
     ref_2t = 2*repspertau*timemeasured*(d(6))*1e3;
...
length(unique(y));
 ...
 z2noise(:,1) = sqrt(1./(z2noise(:,1).^(-2) + z2noise(:,2).^(-2)));
 z2noise_2t(:,1) = sqrt(1./(z2noise_2t(:,1).^(-2) + z2noise_2t(:,2).^(-2)));
  z2noise(:,2) = [];
  z2noise_2t(:,2) = [];
...
T1sigma_t = abs((T1max - T1min)./2)+ (T1max-T1min == 0).*( T1max.*(T1max>=T1min) + T1min.*(T1min<T1max));
 ...
 T1max_2t =  -2*tau./log(1-T1max_2t./contrast);
  ...
  figure(1000);  
  %errorbar(y1,T1_w,T1_w_sigma)
  errorbar(y1,T1_w,T1_w_sigma)
  
  figure(5000);
  errorbar(z2,z2noise);
  figure(5001);
  errorbar(z_2t22,z2noise_2t)
  %imagesc(x1,y1,z2);
  % colormap(bone(256));
  %     h=  colorbar;
  %   xlabel('Position (nm)');
...
%errorbar(y1,T1(:,5),T1sigma_t(:,5),'-r');
 ...
 save T11Dscandata4tau.out T1_w -ASCII
 save T11Derr4tau.out T1_w_sigma -ASCII
