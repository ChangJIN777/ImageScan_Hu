function scan_ps_pillar_z_range_T_ptp_0t0_nPoint( )
...
nx=1; ny=1;
nz = 10; %Number of times to visit each height point
setptFrac = 0.92; % fraction of the free R that the setpoint should be
forceSetpt = 36.1*10^-3; %in volts
Pgain = 0.3; % V/Vrms
...
safeLiftZ_endPixel_V = 1.6; % 2um safelift
...
zrange_nm = [1,15,25,50,200];%[10,50, 200, 1000];%[10,13,18,25,30,35,40,50,75,100,175,250,300,400,500];%[10,30,60,90,120,150,200,400,600,1000]; %[6,10,20,30,40,50,75,100,125,150,200,300];
yrange_ns_omega = [4000,4000,4000,4000,4000]*10^3+500;%[1000,2000 ,4000,4000 ]*10^3+500;%[1000,1000,1200,1200,1400,1800,1800,2000,2400,3500,4500,7000,7000,8000,10000]*10^3+10;%[800,800,800,1000,800,1000,1000,1200,1500,1500,2500,4000]*10^3+10;%[500,500,600,600,700,700,800,800,1000,1000,1000,1200]*10^3;2001000*ones(1,length(zrange_nm));%[1000010,1000010,];%
yrange_ns_gamma = [2000,2000,2000,2000,2000]*10^3+500;%[2000, 2000, 2000, 2000]*10^3+500;
...
zpts_V = zpts_nm*0.0008; 
numTauPoints = 17; % same as in ESR GUI
numc=2;
...
for ka=1:(nz_all*numSequences)
    ...
    if (bTipTrackXY)
     ...
     if (mod(k,galvoTrackPeriod)==0)
       ...
       if is_omega ==1
           %set(esr_handles.fileboxPulseSequence, 'String', 'C:\Users\lab\Documents\MATLAB\ImageScan\NewPulseBlaster\SavedPulseSequences\inUse_sequences\T1_optical_2sig1c_varT.esr');
       ...
       set(esr_handles.writeDataFreq,'UserData',1);
        ...
        %this guarantees that I go 2um above the sample each time to
        %measure tip signal amplitude
        currentDefaultOut = currentDefaultOut-zpts_V(k)+safeLiftZ_endPixel_V;
        ziDAQ('setDouble',['/',zdevice,'/PIDS/0/OUTPUTDEFAULT'],currentDefaultOut);
        
        %disable PID for the 1nm data point
        if (zpts_nm(k)==1)
            %disble PID, at this point the default out is already set to
            %2um from the surface
            ziDAQ('setInt',['/',zdevice,'/PIDS/0/ENABLE'],0);
        end
        
        pause(2)  % good to wait till the amplitude settles. otherwise the amplitude set point can start going down gradually
