...
%                         pulseNumBit1, pulseNumBit2);
        ...
        function PerformSequence(obj,esrGUI)
           ...
           obj.imageScanHandles.bUseSRS2 =0; % got rid of srs2, don't want to restart matlab handles. 
           %Dolev Bluvstein -- Oct 27, 2018. Put in srs 2 again. Turned this from 0 to 1.
           obj.imageScanHandles.bUseSRS3 =0; 
           %Dolev,Zhiran ---Jan 30 2019, Adding another srs 3, just copy everthing.
           
          'time setting'
          tic
            bUseSRS2 = obj.imageScanHandles.bUseSRS2;
            bUseSRS3 = obj.imageScanHandles.bUseSRS3;
          bUseSRS2=0;
            %Dolev Bluvstein -- Oct 27, 2018. Put in srs 2 again. Turned this from 0 to 1.
          bUseSRS3=0; 
            ...
            if bUseSRS2 && bUseSRS3 % Zhiran, Jan 30 2019, I don't understand this part of the code, so I just keep it.
                     
                if get(esrGUI.checkboxUsePhaseNoiseModulationB,'Value')  
                    obj.gesr.setPhaseNoiseB(esrGUI,1); 
                else
                    % 4/20/16 for now don't need any IQ code for SRS2
                   % if get(esrGUI.checkboxIQEnabled,'Value')
                   %     obj.gesr.IQmodulationON(1);
            ...
            if bUseSRS3
                fopen(obj.srs3);
                % Ntype selected
               fprintf(obj.srs3, ['ENBL ', '0']);
                fprintf(obj.srs3, ['ENBR ', '1']);
                fclose(obj.srs3);
            end
...
%                             tempXTau,refCounts2,'b-.');
            ...
            if obj.imageScanHandles.bUseSRS3
                fopen(obj.srs3);
                fprintf(obj.srs3, ['ENBR ', '0']); % turn off the N RF output
                fclose(obj.srs3);
            end
        ...
        function PerformPulsedESRSequence(obj, esrGUI)
          ...
          obj.imageScanHandles.bUseSRS2 =0; % got rid of srs2, don't want to restart matlab handles. 
           %Dolev Bluvstein -- Oct 27, 2018. Put in srs 2 again. Turned this from 0 to 1.
          obj.imageScanHandles.bUseSRS3 =0; 
           %Dolev Bluvstein, Zhiran -- Jan 30, 2019. 


          bUseSRS2 = obj.imageScanHandles.bUseSRS2;
          bUseSRS2=0;  %Dolev Bluvstein -- Oct 27, 2018. Put in srs 2 again. Turned this from 0 to 1.
            %obj.srs = gpib('ni', 0, 24);
