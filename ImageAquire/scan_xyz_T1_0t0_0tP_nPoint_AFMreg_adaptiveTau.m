function scan_xyz_T1_0t0_0tP_nPoint_AFMreg_adaptiveTau( )

%perform image registration 
%XYZ scan with scan style similar to Imagescan
%also implemented adaptive tau points where we go to the next x,y point if
%the current x,y,z point gives a good contrast
...
Force_set_pnt = 59.1e-3; %chnage this each time. used only during first AFM scan
...
% tracking
displacement_AFM_T1 = [0.0,0]*10000;
...
image_regis_period= 2000 ; % 2 hours
...
%set scan speed
afm_scan_speed = 25 ; %in hertz
mDAC('set_scan_speed',afm_scan_speed);

%set number of points in x and y
afm_scan_x_points_numb = 125;
afm_scan_y_points_numb = 125;
...
for ix = 1:nx    
    for iy = 1:ny       
        for iz = 1:nz
            i = i+1;
            xyz_points(i,1) = x_points(ix);
            xyz_points(i,2) = y_points(iy);    
            xyz_points (i,3) = z_points(iz);
        end       
    end    
end
...
T1_intrinsic = 6000 ;% 6.6ms need to find these values experimentally and put here.
T1_engaged = 3500 ; %0.48ms
T1_250nm_away = 5000;
...
tau_maxs =  round(T1_z_total)*1000 +1000 ;%in nanosec now
...
for current_cycle = 1:numb_cycles

    current_point =0 ;  
    
while current_point < nx*ny*nz
    ...
    if (bTipTrackXY)
     ...
     if (mod(AFM_galvo_track,galvoTrackPeriod)==0)
        ...
        dx = current_x_val - xyz_points(current_point,1);
        dy = current_y_val - xyz_points(current_point,2);
        current_x_val = xyz_points(current_point,1); 
        current_y_val = xyz_points(current_point,2);
       ...
       outSamples = zeros(1,numSurfAverages);
        ...
        outTotal = mean(outSamples);
        currentDefaultOut = outTotal+ xyz_points(current_point,3)*(0.8e-3);
        
       
        ziDAQ('setDouble',['/',zdevice,'/PIDS/0/OUTPUTDEFAULT'],currentDefaultOut);
        
        
        if xyz_points(current_point,3) > 1
            ziDAQ('setInt',['/',zdevice,'/PIDS/0/ENABLE'],0);
        end
       ...
       set(esr_handles.tauEnd,'String',num2str(tau_maxs(current_point))); 
       ...
       ['pulse sequence ', num2str(AFM_galvo_track), ' started']
        ...
        set(Img_handles.buttonAOMOn, 'Value', 1) ;
            ...
            % retract tip always to the same height above the surface.
            ziDAQ('setDouble',['/',zdevice,'/PIDS/0/OUTPUTDEFAULT'],currentDefaultOut-xyz_points(current_point,3)*(0.8e-3) +safeLiftZ_endPixel_V);
            ziDAQ('setInt',['/',zdevice,'/PIDS/0/ENABLE'],0);
            pause(0.2);
        ...
        xs = xyz_points(current_point,1);% 
        ys = xyz_points(current_point,2);
        zs = xyz_points(current_point,3);
        
        znmSurf = outTotal*1000;
        scanSaveArray = [xs,ys,zs,znmSurf,scanTauPLTriplets_all];
        %----------------------------------------------
 ...
 % ************** code to re take AFM image 
  %turn on laser b4 AFM scan
   ...
   % and engage
   mDAC('move_tip_laser', -xyz_points(current_point,1) -scan_size_x*10000/2 - displacement_AFM_T1(1),-xyz_points(current_point,2)-scan_size_y*10000/2 - displacement_AFM_T1(2),0,0);
   pause(2);
   ...
   for jx = 0:(afm_scan_x_points_numb-1)
       for jy =0:(afm_scan_y_points_numb-1)
           initial_im(jx+1,jy+1) = initial_image_PL(jx*afm_scan_x_points_numb+jy+1);
           second_im(jx+1,jy+1) = second_image_PL(jx*afm_scan_x_points_numb+jy+1);
       end
   end
...
'the drift in y is'
 ...
 drift_nm_y = drift_V_y*10000;
  ...
  pause(10);
   ...
   if (abs(drift_nm_x) < 350 & abs(drift_nm_y) < 350)
  'move tip'
   ...
   mDAC('set_scan_center_y',initial_pos(2));
       ...
       'drift distances greater than 350nm. so not going to change x,y';
   end
