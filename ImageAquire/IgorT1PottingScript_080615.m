...
%%
igorDateFull = 'C:\Users\lab\Documents\Data\2015_Aug_06\';%get(handles.esrSavePath,'String');
igorDateTrunc = igorDateFull((end-6):(end-1));
latestIndex = num2str(0);%num2str(str2num(get(handles.numCompleted,'String'))-2);

% parameters to change with new data sets
numgraphs = 36;
prefix = 'G13_zscan4_';%'autoZ_NVB';
suffix = '';
repsPerTau = 300000;
pi_time = 0;
...
for i=1:numgraphs
    if i < 9
        igorFolderName = [prefix '1000' num2str(i+2)];
    else
        igorFolderName = [prefix '100' num2str(i+2)];      
    end
    x1= num2str(xmat(i));
