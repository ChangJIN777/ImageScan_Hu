function scan_ps_pillar_xy_z_T99( )
...
nx = 11; %Number of points along x axis
ny = 11; %y axis
nz=1;
...
yrange_ns = [2000,2000]*10^3;%[500,600,600,600,600,600,600,700,700,800,800,800]*10^3;%[500,500,600,600,700,700,800,800,1000,1000,1000,1200]*10^3;
...
scan_size_x = 0.2;
scan_size_y = 0.2;
%set scan size to 2 microns in x
mDAC('set_scan_size_x', scan_size_x);

%set scan size to 2 microns in y
...
afm_scan_x_points_numb = 10;
afm_scan_y_points_numb = 10;
...
initial_scan=importdata('C:/AFM/scans/7352/007352.scan');
file_num = 7352;
    ...
    for i=1:nx
        ...
        if (bTipTrackXY)
            ...
            for k=1:nz   
               ...
               %zDAC ZETTINGS COMMENTED OUT BY AMILA
...
%                 outTotalAfter = mean(outSamples);
    ...
    %if required number of steps since last routine is done, then so a
    %afm-pl scan
   ...
   %scan center is current tip x,y
    ...
    file_num = file_num+1;
    file_path = strcat ('C:/AFM/scans/', num2str(file_num),'/00',num2str(file_num),'.scan'); 
    second_scan=importdata(file_path);
...
%Amila commented out the zurich code below
%ziDAQ('setDouble',['/',zdevice,'/PIDS/0/OUTPUTDEFAULT'],currentDefaultOut);
%ziDAQ('setInt',['/',zdevice,'/PIDS/0/ENABLE'],0);
