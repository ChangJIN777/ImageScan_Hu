...
if (exist('./PulseBlaster/SpinAPI', 'dir') == 7)
    addpath('./PulseBlaster/SpinAPI/');
else
...
% since the pulseblaster is also used in ImageScan, there can be some
% interference - check if global variable Img_handles exists (if ImageScan
% has been opened in this session of Matlab (04/2019 SB)
global Img_handles
Img_exists  = ~isempty(Img_handles); %&& Img_handles;
if ~Img_exists
   Img_handles.PulseInterpreter.SPINAPI_DLL_NAME = SPINAPI_DLL_NAME;
end
...
%     error('Cannot find C:\SpinCore\SpinAPI\dll\spinapi.h'); assume this
%     means it is already loaded instead of throwing an error
end
